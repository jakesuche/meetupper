...
export default{
  ...
  methods:{
    ...
    updateUser({user, done}){
      const sound = 'http://soundbible.com/mp3/Air Plane Ding-SoundBible.com-496729130.mp3'
      this.$store.dispatch('auth/updateUser', user)
      .then(()=>{
        var audio = new Audio(sound)
        audio.play()
        this.$vs.notify({title:'success',text:'Profile successfully updated',color:'primary',icon:'mail',position:'top-center'})
        done()
      })
      .catch((err)=>{
        console.log(err)
