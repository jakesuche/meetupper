      ...
      <div class="hero-body">
        <div class="container">
          ...
          <div class="field">
            ...
            <input v-model="meetup.title" class="title input w-50"
                   type="text">
          </div>
          <article class="media v-center">
            <figure class="media-left">
              <p class="image is-64x64" :src="meetupCreator.avatar || {}">
...
export default {
    ...
    methods:{
        ...
        fetchMeetupByIdHandler(){
            this.fetchMeetupBy(this.meetupId)
            .then(meetup=>{
               if(meetup.meetupCreator._id !== this.authUser._id){
                this.$router.push('/')
                   this.$vs.notify({title:'Error',text:'Sorry! Operation not possible',color:'danger'})
                   
               }
                this.creatorName = meetup['meetupCreator'].name
